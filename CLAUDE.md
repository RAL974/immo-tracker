...
## Pseudonymisation d'employes.json — résolution de noms via le Worker (août 2026)

*Raisonnement complet et statut détaillé : `04_HISTORIQUE_DECISIONS.md` et `SECURITE_ETAT.md`. Point de départ : `employes.json`/`immos.json` sont documentés depuis l'audit du 9-10 août comme des catalogues publics contenant des données personnelles (noms des ~99 salariés) — la direction a demandé de corriger ce point. `immos.json` ne contient aucune donnée personnelle (vérifié) ; seul `employes.json` était concerné.*

**✅ Étapes 1 et 2 faites.** `employes.json` est désormais réduit à `Code`+`Poste` (plus de champ `Nom`) et `?employes=1` (Worker) ne renvoie plus de champ `nom` — la résolution passe exclusivement par le nouvel endpoint dédié `?noms_employes=1` décrit plus bas. **Point encore ouvert, volontairement non traité** : les noms déjà présents dans l'historique git d'`employes.json` (commité avec les noms depuis le début du projet) — voir `SECURITE_ETAT.md` § Limites assumées.
...
1. **Fait, déployé, vérifié en conditions réelles** : nouvel endpoint + cache ajoutés, `chargerEmployes()` (app.js) et `loadEmployes()` (dashboard.html) rebranchés pour préférer le nom résolu par ce nouveau mécanisme — avec repli transitoire sur l'ancien champ nom de `?employes=1`/`employes.json` tant que l'étape 2 n'était pas faite. Vérifié par des tests automatisés puis, après déploiement, en conditions réelles contre l'environnement de recette (`https://ral974.github.io/immo-tracker/staging/`, Worker `immo-proxy-staging` réel, données fictives `FIC1`…`FIC9`) : résolution effectivement fonctionnelle de bout en bout, rejet 403 confirmé sans origine reconnue, `employesList`/`employesByCode`/`nomEmploye()` (dashboard) et les listes déroulantes (PWA) alimentés par le nom résolu.
...
## Mode simplifié PWA (roadmap item E, ajouté août 2026)

*Le référent matériel à Mayotte (rôle `Logistique_Mayotte`) est peu à l'aise avec l'informatique (imprime des documents, envoie des photos par email plutôt que d'utiliser l'app). Objectif : un accueil réduit à 3 actions, sans rien changer côté serveur. Voir `04_HISTORIQUE_DECISIONS.md` pour le cadrage complet.*

**Couche de présentation uniquement — confirmé** : les 3 écrans du mode simplifié appellent exactement les mêmes actions Worker que le mode normal (`valider`, `transfert`, `declarer_panne`, `creer_retour_direct_garant` via `confirmerAvecEtat`), avec les mêmes règles de droits. Aucun nouvel endpoint, aucune nouvelle liste SharePoint.

**Déclenchement — nouvelle capacité `ROLE_CAPS.modeSimplifie`** (`app.js`), posée uniquement sur `Logistique_Mayotte`. `modeSimplifieActif()` (= `caps(code).modeSimplifie` et pas de bascule "Autres actions" en cours) pilote l'affichage dans `afficherEmploye()`, déjà le point qui adapte l'accueil au rôle — automatique, sans action de l'utilisateur. Extensible à d'autres rôles plus tard en ajoutant `modeSimplifie:true` à leur entrée `ROLE_CAPS`, sans autre changement.

**"Autres actions"** : lien texte discret en bas de l'accueil simplifié (`demanderAutresActions()`), confirmation "Voir toutes les actions ?" avant bascule. La bascule est stockée dans `S.modeSimplifieOverride`, **en mémoire seulement, jamais persistée** (ni `localStorage`, ni SharePoint) — au prochain chargement de page ou après "Changer d'utilisateur", retour automatique au mode simplifié. Le mot "mode" ou tout vocabulaire technique a été volontairement évité dans le libellé visible (retour direct de William : le public cible ne doit pas voir de jargon de développeur).

**Un seul `#screen-accueil`, contenu reconstruit selon le mode** — plutôt qu'un second écran séparé, l'accueil existant contient deux blocs (`#accueil-normal` inchangé, `#accueil-simple` nouveau), affichage basculé par `afficherEmploye()`. Les ~15 endroits du code qui font `showScreen('screen-accueil')` (retours, annulations) continuent de fonctionner sans modification, quel que soit le mode actif.

**Les 3 actions** :
- **✅ Je reçois** — couvre volontairement deux cas, pas un seul (repéré au cadrage, confirmé avec William) :
  1. Valider un transfert en attente (quelqu'un a déclaré depuis son propre téléphone) — réutilise `screen-transferts-attente`/`screen-accepter`/action `valider`, inchangés.
  2. "📷 Autre chose à réceptionner" — réception directe et immédiate (matériel HS remis en main propre, matériel d'un employé sortant...) : réutilise le flux `Retour dépôt` existant (`startMouvement('Retour')`), qui **s'auto-valide déjà** puisque `Logistique_Mayotte` est garant (mécanisme "retour direct par un garant" déjà en place, pas une nouveauté de ce chantier).

  Un mouvement `Retour` ne peut être déclaré que par la personne qui détient physiquement le matériel sur son propre téléphone (règle d'identité déjà en place) — `Logistique_Mayotte` ne peut donc jamais scanner elle-même un objet détenu par quelqu'un d'autre pour le "recevoir" directement à sa place ; elle ne peut que **valider** ce que l'autre a déclaré (cas 1) ou déclarer elle-même un retour dont elle est la détentrice physique du moment (cas 2). Il n'existe donc pas de 3e cas "transfert entrant" en dehors de ces deux chemins.
- **📤 Je rends** — réutilise exactement le flux `Transfert` existant (`startMouvement('Transfert')` : scan → receveur → état → confirmation). L'option "Scanner la carte BTP du receveur" (déjà la plus simple, déjà existante) reste le choix principal.
- **🔧 Signaler une panne** — réutilise exactement `scannerPourPanne()` → `screen-signaler-panne`, déjà minimal (1 champ obligatoire + photo optionnelle), aucune modification.

**5 états conservés, pas réduits** : décision explicite de William — malgré l'objectif "moins de texte", les 5 états (Neuf/Bon état/Usé/Abîmé/Hors service) restent tous disponibles en mode simplifié, en grille de gros boutons colorés à la place du menu déroulant (réutilise les classes CSS `.etat-neuf/.etat-bon/.etat-use/.etat-abime/.etat-horsservice`, déjà définies dans `style.css` mais orphelines jusqu'ici). "Hors service" en particulier est nécessaire pour le cas concret du matériel HS remis en main propre (cas 2 de "Je reçois").
...
## Finition visuelle du dashboard — tableaux, header, sidebar, résidus mode sombre (12 août 2026)

Session de finition pure (aucun changement de comportement métier, gabarits imprimables EPI/Outillage/Relevé de sortie vérifiés octet pour octet inchangés en fin de session), demandée après plusieurs allers-retours sur la refonte visuelle et le mode sombre.

- **Colonnes de statut/badge bornées** : dans un tableau à mise en page automatique étiré à 100% (`table{width:100%}`), une cellule ne contenant qu'un badge/statut recevait sa propre part de l'espace excédentaire au même titre qu'une colonne de texte libre voisine, l'écartant visuellement du contenu auquel elle se rapporte — cas témoin signalé par William : Historique EPI, où NOM et DOTATION doivent rester visuellement associés. Corrigé par une règle générique dans `design-system.css` (`.tbl-wrap td:has(> .badge){width:1%;white-space:nowrap}`) plutôt qu'un correctif par tableau : force le navigateur à ne donner à ces colonnes que le strict nécessaire, le surplus allant aux colonnes de texte libre adjacentes qui l'absorbent mieux visuellement.
- **`font-variant-numeric:tabular-nums`** ajouté sur `.tbl-wrap td` (chiffres alignés dans les colonnes de montants/quantités/dates).
- **Zébrage adouci** : la teinte chaude/dorée d'origine (`--ds-zebra`, un hex propre par thème) remplacée par une simple surimpression neutre à faible opacité (~3.5% clair, ~4.5% sombre) via `rgba()` — s'adapte seule au thème (encre sombre sur fond clair, encre claire sur fond sombre) sans recalibrer une valeur séparée par thème. Vérifié dans les deux thèmes.
- **Liens dans les tableaux** (`.link-im`/`.link-emp`) : couleur d'accent (`--ds-accent`, remplace `--blue` — signale l'affordance "cliquable" sans se confondre avec les titres/valeurs) et soulignement au survol seulement, plus de soulignement permanent. Piège rencontré en vérifiant dans le navigateur : la règle ajoutée dans `design-system.css` était neutralisée par la règle homonyme déjà présente dans le `<style>` inline de `dashboard.html`, chargé après et donc prioritaire à spécificité égale — corrigée en éditant directement le bloc existant dans `dashboard.html` plutôt que d'empiler une redéfinition qui perd la cascade. Nouveau modificateur `.link-plain` (sélecteur composé `.link-emp.link-plain` pour rester prioritaire quel que soit l'ordre) : couvre le cas d'une ligne à deux liens vers la même fiche (ex. colonne Code + colonne Nom dans l'Historique EPI/Outillage, `lienEmpSiConnu()` appelée deux fois) — le clic reste actif sur les deux (même classe `link-emp`, même délégation d'événement), mais seul le nom se lit comme un lien ; le code redevient du texte normal (`--ds-text-muted`) à côté.
- **Header — hiérarchie des actions** : "Excel" et "Actualiser" (fonds pleins vert/orange) passent en boutons discrets (translucides sur le fond du header), cohérents avec le bouton de bascule de thème déjà traité ainsi — plus aucun bouton à fond plein coloré en compétition dans le header. Le timestamp "Actualisé …" (`#last-upd`, span séparé) est retiré ; l'information passe en `title` du bouton Actualiser (infobulle au survol), ce qui libère de la place sans perdre l'info.
- **Sidebar — badges honnêtes** : nouvelle fonction `setNavBadge(id,count)` + règle CSS `.nav-badge:empty{display:none}` — un badge à 0 ou pas encore chargé (`textContent` vide) ne s'affiche plus, au lieu d'un tiret (`—`) permanent dont le sens n'était pas clair pour William. Appliqué aux 8 badges (circulation, dépôt, transferts, réservations, documents FDS, maintenance, absences, alertes) ; les deux derniers (Documents FDS, Maintenance) étaient des tirets "chargement paresseux" (l'onglet n'a pas encore été ouvert dans la session, voir chargement paresseux documenté plus haut dans ce journal) — ce sont de vrais compteurs, simplement pas encore résolus, et se comportent désormais comme tel (rien tant que non chargé, un vrai nombre une fois l'onglet visité).
- **Mode sombre — champ de recherche du header** : `#gsearch-input` pose un fond blanc translucide (`rgba(255,255,255,.12)`) directement sur `--ds-nav-bg`. En clair, ce fond reste assez lumineux pour lire un champ neutre ; en sombre, `--ds-nav-bg` devient un bordeaux profond et ce même blanc translucide se teinte nettement de rose/mauve au lieu de rester neutre (symptôme signalé par William) — corrigé par un fond opaque neutre (encre `--ds-ink-900`) réservé au thème sombre uniquement (`:root[data-theme="dark"] #gsearch-input`), le mode clair n'étant pas concerné.
- **Audit des couleurs hex codées en dur** : grep systématique hors `<style>` principal (déjà tokenisé) et hors les 3 gabarits imprimables (repérés par plage de lignes et exclus). Sur ~150 occurrences résiduelles trouvées dans le HTML généré côté JS, corrigées celles qui produisaient un rendu réellement incohérent en sombre — tuiles d'info/statut à fond clair fixe (`#E3F2FD`/`#0D47A1`, `#FBE9E7`/`#FFEBEE`, `#E8F5E9`/`#1B5E20`, `#FFF3E0`/`#FFF8E1`/`#E65100`, `#ECEFF1`/`#F5F5F5`, `#F8FAFC`) remplacées par les tokens `--ds-info-*`/`--ds-danger-*`/`--ds-success-*`/`--ds-warning-*`/`--ds-neutral-100`/`--ds-surface-alt` déjà définis et theme-aware, plus quelques bordures/texte muet isolés (`border...#f0f0f0`, `border:2px solid #ddd`, `color:#555`) vers `--ds-border`/`--ds-border-strong`/`--ds-text-muted`. Script Node ponctuel (non conservé dans le dépôt) appliquant les remplacements ligne par ligne en excluant explicitement les plages du `<style>` principal et des 3 gabarits, pour ne jamais risquer d'y introduire une variable CSS qu'un document imprimé isolé (nouvelle fenêtre, sans `design-system.css`) ne pourrait pas résoudre.
  - **Volontairement laissées en l'état** (documenté, pas oublié) : les couleurs de "chips" d'identité — palette Chart.js (`#A67A1E`, `#1E9E5A`...), couleurs de rôle/rang dans `roleInfo`/le tableau Utilisateurs (`#1A3A6B`, `#5E35B1`, `#D84315`...), le badge de site Réunion/Mayotte (teinte bleu/teal dédiée, distincte de la palette sémantique badge), et une poignée de teintes d'accent isolées (`#9C27B0` "chez le prestataire", `#F9A825` garanties, boutons "Outils de maintenance avancés"). Ce sont des systèmes de couleur d'identité visuelle intentionnels, pas des oublis de tokenisation — les retravailler pour le mode sombre est un sujet de redesign à part entière, pas une correction de résidu.
- **Vérifié en navigateur** (données factices injectées côté client, serveur statique local, jamais le Worker de production) : tableau de test reproduisant le cas Historique EPI (largeur de colonne badge mesurée à 90px contre 317px pour la colonne Nom, `getBoundingClientRect`), couleurs de lien et de zébrage vérifiées par `getComputedStyle` dans les deux thèmes, fond du champ de recherche vérifié en sombre (neutre) et en clair (inchangé), `setNavBadge`/`.nav-badge:empty` vérifiés avec des stats factices (0 → masqué, valeur réelle → affiché). `npm run verify` (240/240) avant et après les correctifs de cascade CSS. Les 3 gabarits imprimables confirmés inchangés par `git diff` (aucune plage de lignes touchée).

## Recherche globale → palette de commande (12 août 2026)

- **Demande de William** : transformer le champ de recherche globale existant (`gsearch-input`, raccourci `/`, ajouté en session précédente) en une vraie palette de commande — navigation clavier, groupe "Navigation" pour sauter directement à un onglet/sous-onglet par son nom, matching insensible aux accents/casse. Contrainte explicite : 100% côté client, aucun nouvel appel Worker (les données affichées sont déjà en mémoire pour le reste du dashboard).
- **Matching multi-mots généralisé** : l'ancien code comparait la requête entière (`.includes(q)`) à chaque champ ; remplacé par `gsNormalize()` (strip des accents via `String.normalize('NFD')` + suppression des marques diacritiques, majuscule) et `gsMatchesTokens()` (chaque mot tapé, séparé par des espaces, doit se retrouver quelque part dans le texte cherchable — ordre indifférent). Permet "epi hist" → "EPI › Historique" comme demandé, et retrouve "ANAPOTÉLIVOUA" en tapant "anapotelivoua" sans accent. Appliqué uniformément aux 5 groupes existants (immos/employés/EPI/outillage/brasseurs) en plus du nouveau groupe Navigation, pour un comportement de recherche cohérent partout dans la palette plutôt qu'un cas particulier pour la navigation seule.
- **Piège évité en écrivant la regex de suppression des accents** : `[̀-ͯ]` (plage Unicode des marques diacritiques combinantes) a été construite via `String.fromCharCode()` plutôt qu'écrite comme séquence d'échappement littérale dans le code source — un premier essai avait fini par insérer les caractères combinants eux-mêmes dans le fichier au lieu de leur notation `\u`, ce qui aurait rendu la regex fragile et quasi illisible/non-éditable dans un éditeur de texte normal. Repéré et corrigé avant tout commit.
- **Groupe "Navigation"** (`NAV_INDEX`, nouveau tableau déclaratif) : liste tous les onglets principaux (`data-tab`) et les sous-vues des modules à sous-onglets (EPI, Prime d'outillage, Brasseurs d'air, Matériel IT — même variables de sous-vue déjà en place, `epiVue`/`outilVue`/`brasseursVue`/`matItVue`, réutilisées telles quelles). Sélectionner une entrée pose la sous-vue puis appelle `goTab()` (mécanisme `data-tab` existant, aucun nouveau routeur). L'entrée "Journal d'audit" est marquée `adminOnly` et filtrée hors des résultats pour un non-admin — cohérent avec le fait que ce même nav-item est déjà masqué dans la sidebar pour ces rôles (`appliquerVisibiliteJournal()`) ; les autres modules (EPI, Outillage...) restent volontairement toujours proposés dans la palette même pour un rôle qui n'y a pas accès, exactement comme leur nav-item de sidebar reste toujours visible — l'écran affiche alors son propre message "🔒 Réservé..." déjà en place, dégradation gracieuse cohérente avec le reste de l'app plutôt qu'un nouveau filtre de droits dans la palette.
- **Navigation clavier** : `GSEARCH_ITEMS` (liste à plat reconstruite à chaque rendu, dans l'ordre d'affichage) + `GSEARCH_SEL` (index surligné). ↑/↓ déplacent la sélection avec retour en boucle (dernier→premier et inversement), Entrée active l'élément sélectionné, la souris (survol) resynchronise la même sélection — un seul état partagé entre clavier et souris plutôt que deux mécanismes parallèles. Premier résultat pré-sélectionné automatiquement à chaque frappe, pour qu'Entrée fonctionne immédiatement sans devoir appuyer sur ↓ d'abord.
- **Simplification technique au passage** : les lignes de résultat utilisaient auparavant des `onclick` inline avec les valeurs (code immo, nom...) injectées directement dans l'attribut HTML, échappées à la main (`.replace(/'/g,...)`) pour éviter de casser le HTML si un nom contient un guillemet. Remplacé par des closures JS stockées dans `GSEARCH_ITEMS` (chaque ligne ne porte plus qu'un index `data-gs-idx`) — supprime le besoin d'échappement pour l'attribut (un nom avec une apostrophe, ex. "Maël D'Angelo", ne peut plus jamais casser un `onclick`) et centralise l'activation (clic, Entrée) sur un seul point (`gsActivate(idx)`).
- **Dégradation gracieuse** : le corps de `renderGlobalSearch()` est encadré d'un `try/catch` — une erreur de rendu (ex. donnée inattendue dans un catalogue) est journalée en console mais ne bloque jamais la frappe dans le champ, cohérent avec la contrainte explicite de ne rien casser du comportement actuel si la palette échoue.
- **CSS** : nouvelle classe `.gs-item.sel` (fond `--ds-gold-50` + bordure gauche `--orange`, mêmes tokens que l'indicateur "actif" de la sidebar) — theme-aware sans code dédié, `--ds-gold-50` ayant déjà une valeur distincte en mode sombre (introduite lors de la refonte visuelle du 12 août pour le survol de ligne de tableau).
- **Tests** : nouveau fichier `tests/dashboard.global-search.test.js` — extrait le bloc autonome `gsNormalize`/`gsMatchesTokens` et le littéral `NAV_INDEX` de `dashboard.html` (même principe que `scripts/check-dashboard.js`, qui vérifie la syntaxe du JS inline, mais ici le code est réellement exécuté dans un contexte `vm` isolé plutôt que juste parsé). Couvre la suppression d'accents, le matching multi-mots dans les deux ordres, le cas "aucun token" et le filtrage `adminOnly` du Journal d'audit.
- **Vérifié en navigateur** (données factices injectées directement en JS — code, nom, catalogues EPI/outillage/brasseurs fabriqués — sans toucher au Worker de production) : "epi hist" retrouve bien "EPI › Historique" et pré-sélectionne cette entrée ; "anapotelivoua" (sans accent) retrouve "ANAPOTÉLIVOUA" ; ↑/↓ déplacent la sélection avec retour en boucle correct sur 2 résultats ; activer une entrée EPI/Outillage bascule effectivement l'onglet et la sous-vue attendus (vérifié après résolution de l'appel asynchrone `loadEPI()`/`loadOutillage()`) ; activer un résultat immo ouvre bien la fiche existante sans changer d'onglet ; Échap vide le champ et retire le focus ; le filtre `adminOnly` du Journal d'audit confirmé (absent pour un rôle non-admin, présent pour AIWI). Styles de sélection vérifiés par `getComputedStyle` dans les deux thèmes (fond/bordure distincts en clair et en sombre, tous deux lisibles).
- `npm run verify` : 248/248 (240 existants + 8 nouveaux pour la palette), y compris la vérification de synchronisation `staging/` (`npm run sync:staging` relancé après modification de `dashboard.html`).

## Session UX/UI de la PWA terrain (12 août 2026)

- **Demande de William** : passe de cohérence visuelle sur `index.html`/`app.js`/`style.css` (la PWA terrain, ~97 collaborateurs de chantier, smartphone, plein soleil, gants, réseau intermittent à Mayotte) — 7 points cadrés à l'avance (A à G), zéro changement de logique métier ni d'appel Worker, zéro framework/build. Contrainte rappelée en tête : les deux CSS concurrents documentés (`style.css` + `<style>` inline de `index.html`, ce dernier gagnant la cascade) — toute nouvelle règle regroupée à un seul endroit cohérent avec l'existant (l'inline, déjà source de vérité pour `.btn-*`/`.screen`).
- **A — Cohérence de la couleur primaire** : recensement de tous les boutons d'action de la PWA — la moitié utilisait `btn-secondary` (or) comme CTA, l'autre moitié `btn-primary` (marine), sans règle claire, produisant sur l'accueil exactement le "mur" décrit par William (Transférer or plein / Retour dépôt marine plein / Mon matériel bleu teinté / Réservations-Gestion personnel-Inventaire-Brasseurs en outline à bordure grise avec style inline dupliqué 4 fois). **Décision technique** : ne pas redéfinir globalement `.btn-primary`/`.btn-secondary` (risque de collision avec le mode simplifié PWA, roadmap item E, qui réutilise `.btn-primary` comme une des 3 couleurs déjà validées de son propre design — volontairement laissé inchangé, hors périmètre de cette demande qui ne cite que l'accueil normal et l'exemple "Transférer"). À la place, reclassement écran par écran vers les classes déjà correctes : `btn-secondary` (or) conservé comme unique action primaire par écran, tout le reste unifié sous `btn-tertiary` (au lieu des 3 traitements différents) ou promu en `btn-secondary` quand il s'agissait déjà de l'unique CTA d'un écran auparavant marine (`+ Nouvelle réservation`, `Signaler une absence`, `Envoyer la demande`, `Enregistrer` sur Modifier réservation, `Retour accueil` sur l'écran de confirmation). Les icônes caméra à côté des champs de recherche (`screen-rechercher`, `screen-force-attribution`, `screen-inventaire-stock`, `screen-reserver`, `screen-modifier-resa`) repassées en neutre — ce ne sont pas des CTA concurrents. `Ajouter au bordereau` (Brasseurs) démoté d'or vers neutre : `Valider la sortie` reste la seule action or de l'écran (l'ajout au bordereau est répétable, pas l'écriture finale).
- **Chevauchement d'ombres résolu de fait** : le "mur" de boutons marine avait chacun une ombre large (`box-shadow: 0 4px 14px`, définie dans `style.css`, jamais redéclarée inline donc toujours active) ; en repassant ces boutons en `btn-tertiary` (ombre subtile `0 1px 4px`), l'effet de bordure floue qui se chevauchait entre deux boutons empilés avec un `gap` de 10-12px disparaît sans avoir eu besoin de toucher aux valeurs de `box-shadow` elles-mêmes.
- **B — Contrôle segmenté** : nouvelle classe réutilisable `.segmented`/`.seg-btn` (conteneur unique, `min-height:48px`), appliquée à la bascule Sortie/Transfert du module Brasseurs (seul toggle binaire de ce type dans la PWA — le mode "demande par catégorie" de l'écran réservation est une case à cocher, pas un toggle, et la sélection de destination Brasseurs Chantier/Maintenance/Client est un choix à 3 options non binaire, tous deux hors périmètre de ce composant). **État actif marine, pas or** : un toggle de mode est un élément "sélectionné" (la case explicitement citée par William dans la règle de couleur), pas une action d'écriture — le garder hors de la couleur or évite toute concurrence visuelle avec le bouton de validation or du même écran (`Valider la sortie`/`Valider le transfert`). Nouvelle fonction JS `marquerSegmentActif()` (simple `classList.toggle`), distincte de `marquerBoutonDestinationSelectionne()` (qui reste en style inline pour les boutons `.btn-outline`, à cause du même piège de spécificité CSS déjà documenté en commentaire dans le code).
- **C — Badges de stock sémantiques** : dans la liste de références du module Brasseurs (PWA), le badge `<span class="chantier-tag">X en stock</span>` (même style neutre quel que soit le niveau de stock) remplacé par `badgeStockBrasseur()` — badge vert (`stock-badge-ok`) si stock disponible, badge gris atténué (`stock-badge-zero`) si rupture. Style seulement, le `onclick` de sélection de la ligne est inchangé (une référence en rupture reste sélectionnable, pour ne pas introduire un nouveau blocage non demandé).
- **D — Micro-corrections formulaires** :
  - **Bug réel trouvé par mesure, pas par relecture** : le champ IMMO + bouton caméra du formulaire de réservation (`#resa-immo-bloc`) mesurait 286px de large contre 343px pour les autres champs du même écran (ex. le sélecteur Catégorie juste au-dessus) — un décalage de 28px de chaque côté, clairement visible. Cause : `.screen` est un flex-column avec `align-items:center` (les items ne s'étirent pas par défaut) ; les champs simples (`.select-input`, `.field-label`) ont une règle `width:100%` explicite qui les étire correctement, mais `#resa-immo-bloc` est un `<div>` nu sans largeur déclarée — en tant qu'item flex, il se réduit à la taille de son contenu ("fit-content"), lequel est tiré vers le bas par le bouton caméra à taille fixe dans la `.search-row` imbriquée à l'intérieur. Vérifié comparativement : le même schéma `.search-row` + bouton caméra, quand il est un enfant DIRECT de `.screen` (`screen-rechercher`), s'étire correctement à 343px — la div d'enrobage supplémentaire est bien la cause. Corrigé par une règle ciblée `#resa-immo-bloc, #resa-cat-bloc { width:100% }` plutôt qu'une règle générale sur tous les `<div>` de `.screen` (trop large, risque de régression ailleurs jamais audité).
  - Cases à cocher : la case "demande par catégorie" (réservation) mesurait déjà 48px de hauteur de zone cliquable (label inclus, vérifié par `getBoundingClientRect`) mais une case à cocher visuellement plus petite (20×20) que celle de la campagne d'inventaire (22×22, déjà correcte) — agrandie à 22×22 pour la cohérence. La case "chute de câble" (inventaire) était déjà conforme, aucune modification nécessaire.
  - **Liseré violet** : recherché dans tout le code (grep sur `violet`/`purple`, sur les codes hexadécimaux commençant par une teinte violette, sur les `accent-color` calculés des champs natifs `checkbox`/`date`) — **aucune trace trouvée**, aucune couleur violette n'existe nulle part dans `style.css`, le `<style>` inline ou `design-system.css`. Non reproduit non plus par inspection des styles calculés. Signalé à William plutôt qu'un correctif inventé à l'aveugle : si le liseré est réel, une capture d'écran de l'écran concerné serait nécessaire pour le localiser (pourrait être un artefact du navigateur/OS du poste concerné, hors du code de l'app).
- **E — Indicateur réseau permanent** : le bandeau hors-ligne (`#offline-banner`, fond gris neutre, texte informatif avec horodatage de dernière synchro) existait déjà d'une session précédente et remplissait déjà l'essentiel de la demande — conservé tel quel plutôt que remplacé par le texte générique suggéré ("Hors ligne — consultation seule"), qui aurait fait perdre l'horodatage utile déjà en place. Seul ajout : une confirmation brève ("✅ Connexion rétablie", toast 2,5s) au retour réseau, déclenchée uniquement sur une vraie transition hors-ligne → en ligne (jamais affichée si la page a toujours été en ligne, via un indicateur `_etaitHorsLigne`). Vérifié en simulant `navigator.onLine` false puis true dans le navigateur.
- **F — Feedback de scan renforcé** : nouveau point de passage unique `scanFlash(ok)` (surimpression plein écran verte/rouge ~150ms, `pointer-events:none`) + `scanFail()` (double vibration courte, dégradation silencieuse si `navigator.vibrate` absent — iOS Safari notamment, déjà le comportement de `vib()` existant). Succès : ajouté dans `onDecode` (le point de passage déjà unique pour tout scan réussi, caméra ou saisie manuelle confondues, documenté dans une session précédente). **Échec, gap réel trouvé en auditant le code plutôt que supposé** : plusieurs écrans avaient déjà un `toast('...', 'error')` sur un scan de mauvais type (carte employé attendue mais immo scannée, etc.) — `scanFail()` ajouté à chacun de ces points existants (`onScanActivation`, `scannerCarteReceveur`, `demarrerValidation`, `scannerPourForce`, `scannerPourResa`). Deux écrans (`onScanImmo`, `scannerModifImmo`) n'avaient en réalité **aucun** feedback du tout sur un mauvais type de code scanné (un `return` silencieux) — un vrai trou dans l'UX existante, pas seulement l'absence du nouveau flash : corrigé en ajoutant le même `toast('Ce n'est pas une immo', 'error')` que les écrans sœurs, plus `scanFail()`. Aucun son ajouté (chantier bruyant, demande explicite de William).
- **G — Confirmations récapitulatives grosse typo** : nouvelle classe partagée `.recap-title` (800/19px, remplace ~15px/700 dispersé en style inline sur 8 éléments) appliquée aux petits titres "Désignation → Destination" déjà existants en haut des formulaires à un seul bouton de validation (`etat-immo-info`, `accepter-texte`, `receveur-immo-info`, `resa-immo-info`, `panne-immo-titre`, `resolution-immo-titre`, `retrait-immo-titre`, `modif-resa-info`) — ces textes combinaient déjà QUOI (désignation+code) et QUI/OÙ (destination) sur une seule ligne, simplement en trop petit. Le bouton de validation or unique par écran était déjà garanti après le travail du point A (vérifié automatiquement : aucun écran n'a plus de 2 `.btn-secondary` visibles simultanément — les 2 seules exceptions, `Valider la sortie`/`Générer le bon` sur l'écran Brasseurs, sont dans des panneaux qui se masquent mutuellement, jamais visibles en même temps). **Écrans d'écriture majeure sans étape de confirmation dédiée, non créée (hors périmètre explicite de la demande)** : Attribution forcée (`screen-force-attribution`, le formulaire entier reste visible, pas de recap séparé), Signaler une absence (`screen-signaler-absence`, aucun texte récapitulatif avant le bouton). L'ajout d'une ligne d'inventaire stock n'a pas été traité comme "majeur" (action répétable, faible enjeu, corrigeable tant que la campagne est en cours).
- **Vérifié en navigateur** (serveur statique local, données factices injectées côté client — aucun appel en écriture au Worker de production) : un seul bouton or visible par écran sur l'ensemble des écrans testés (accueil, receveur, réservation, signaler panne), ombre du "Retour dépôt" redevenue subtile, alignement du champ Immo réservation passé de 286px à 343px (identique aux champs voisins), contrôle segmenté Sortie/Transfert (actif marine, inactif blanc), badges de stock verts/gris selon la disponibilité, overlay de flash scan bascule correctement `flash-ok`/`flash-fail`, case à cocher réservation à 22×22px dans une zone cliquable de 48px, bandeau hors-ligne + confirmation de reconnexion, aucun débordement horizontal à 375px sur les écrans modifiés.
- `npm run verify` : 248/248 (aucun test nouveau requis — changements de présentation PWA uniquement, pas de nouvelle action Worker ni de nouvelle logique testable côté serveur), `npm run sync:staging` relancé après modification de `index.html`/`app.js`.

## Sortie de stock Brasseurs : blocage de sélection à stock zéro, exception garant (12 août 2026)

- **Révise une décision de la session précédente** ("Session UX/UI de la PWA terrain", point C ci-dessus) : le badge de stock sémantique (vert/gris) avait été livré volontairement sans changement de comportement — "une référence en rupture reste sélectionnable, pour ne pas introduire un nouveau blocage non demandé". William a maintenant demandé explicitement ce blocage, en mode Sortie seulement.
- **Règle métier** : en mode **Sortie** du module Brasseurs (PWA), une référence à stock 0 (au dépôt source choisi) devient non sélectionnable pour les profils standards — au tap, rien n'est sélectionné, toast d'erreur "Rupture de stock — sortie impossible pour cette référence." **Exception** : les profils garant du module (Admin/Logistique/Logistique_Mayotte) gardent la possibilité de sélectionner et valider malgré un stock à zéro — cas de régularisation d'un stock théorique faux (le stock calculé par l'app peut être erroné si une sortie physique a eu lieu sans être ressaisie) — avec un avertissement non bloquant à la sélection : "Stock théorique à 0 — la sortie créera un écart⚠️". **Mode Transfert inchangé** : aucune restriction de sélection, comme avant.
- **Aucune nouvelle capacité `ROLE_CAPS`** : réutilise `SB.estGarant` (déjà calculé à l'ouverture de l'écran via `estGarant(S.employe.code)`, capacité `garant` — Admin/Logistique/Logistique_Mayotte), la même population qui pilote déjà le mode avancé (Transfert inter-dépôts, destination Client) livré le 11/08. Décision cohérente avec le cadrage initial du module ("achats ou logistique... gestionnaire de dépôt ou atelier").
- **Défense en profondeur côté Worker** : le garde-fou de stock négatif existait déjà dans `sortie_stock_brasseur_pwa` (`stock_insuffisant`, appliqué sans distinction de rôle depuis la livraison initiale du 11/08) — la sélection bloquée côté PWA n'aurait donc été qu'un confort d'UI sans lui. Aligné sur la règle ci-dessus : `estGarantBrasseurServeur(codeEmploye)` (déjà utilisée pour la destination Négoce) est désormais calculée systématiquement (pas seulement pour Négoce) et lève ce garde-fou pour la population garant. **Portée volontairement généralisée à tout stock insuffisant, pas seulement à un stock strictement nul** : côté serveur, "stock à zéro" et "stock partiellement insuffisant" sont la même condition (`stockActuel - demande < 0`) — les distinguer aurait été arbitraire et aurait laissé un garant bloqué dans un cas de régularisation tout aussi légitime (ex. stock affiché à 2, sortie réelle de 5). Les profils standards restent bloqués dans tous les cas, comme avant.
...
## Correctif : bouton "Confirmer" bloqué de façon aléatoire sur la modale de mouvement (13 août 2026)

- **Signalé par William avec capture d'écran** : sur "Créer un transfert" (bouton rapide depuis une fiche immo), le bouton "✅ Confirmer" affichait parfois "…" en permanence, insensible au clic — aucune erreur, aucun toast, la fenêtre ne se fermait pas, aucun mouvement enregistré, aucune trace. Décrit comme intermittent ("des fois ça marche"), avec demande explicite de vérifier si d'autres actions du dashboard partagent le même défaut.
- **Cause racine identifiée en lisant le code, pas en devinant** : `#act-confirm-btn` est **un seul bouton HTML partagé par ~7 flux différents** qui ouvrent la même modale `#act-overlay` (créer un transfert/retour, suivi panne, résoudre panne, réparation/entretien, contre-proposition de réservation, sortie définitive, modifier un transfert en attente). `soumettreAction()` (le flux "Créer un transfert") désactive ce bouton et lui donne le texte "…" pendant l'envoi — mais `fermerAction()`, le point de fermeture commun appelé en cas de succès, ne réinitialisait **que** `.onclick`, jamais `disabled`/`textContent`. Résultat : après un transfert créé **avec succès**, le bouton restait durablement désactivé avec "…" affiché — et un clic sur un bouton HTML `disabled` ne déclenche **aucun** événement `click`, donc aucun code ne s'exécute, aucune erreur n'est possible, rien n'est journalisé. Explique tous les symptômes d'un coup : silence total, modale figée, et un fonctionnement qui semble aléatoire (ça marche au premier essai après chaque rechargement de page, ou juste après un échec — dont le `catch` réinitialisait déjà correctement l'état — mais casse après **chaque succès**).
- **Second défaut trouvé au même endroit, plus insidieux** : `ouvrirActionMouvement()` (l'ouverture de "Créer un transfert/retour") est le **seul** des 7 flux à ne jamais réassigner explicitement `.onclick` à l'ouverture — il suppose que le bouton pointe déjà vers `soumettreAction`. Les 6 autres flux réassignent bien `.onclick` à chaque ouverture (bon réflexe défensif), sauf `ouvrirModifTransfert` (modifier un transfert en attente de validation) dont le flux de succès n'avait **aucun `.catch()`** sur son `fetch` — une panne réseau y laissait une rejection de promesse non gérée, sans jamais appeler `fermerAction()` ni afficher d'erreur, un deuxième cas silencieux du même genre.
- **Corrigé à la racine plutôt qu'au cas par cas** : nouvelle fonction `resetActBtn()` — réinitialise `disabled=false`, `textContent='✅ Confirmer'` et l'indicateur anti-double-clic (`dataset.busy`) en un seul point. Appelée (1) dans `fermerAction()`, le point de fermeture commun à tous les flux (Annuler, clic hors modale, ou succès d'un des 7 flux) — corrige le cas signalé, qui repassait toujours par là ; (2) à l'**ouverture** de chacun des 7 flux (défense en profondeur : même si un futur flux oublie d'appeler `fermerAction()` sur un chemin d'erreur, le prochain flux qui ouvre la modale repart d'un état propre, quel que soit celui laissé par le précédent). `ouvrirActionMouvement()` réassigne désormais aussi explicitement `.onclick=soumettreAction` à l'ouverture, alignée sur le même réflexe défensif que les 6 autres flux — elle ne dépend plus silencieusement de l'état laissé par le dernier flux utilisé. `ouvrirModifTransfert()` reçoit un `.catch()` (erreur affichée, bouton réactivé) et un garde anti-double-clic identique à `resoudrePanneComplet`/`ouvrirEnregistrerReparation`.
- **Vérifié en navigateur, pas seulement relu** (données factices, `fetch` stubbé pour simuler un succès `creer_mouvement_direct` sans toucher au Worker de production) : reproduction fidèle de la séquence signalée — ouverture de "Créer un transfert", clic réel (`btn.click()`) sur un succès, **puis réouverture de la même modale** : avant le correctif ce test aurait montré `disabled:true, text:'…'` (bouton mort) ; après correctif, confirmé `disabled:false, text:'✅ Confirmer', onclick===soumettreAction` — la seconde soumission aboutit normalement (l'overlay se referme). Le nouveau `.catch()` de `ouvrirModifTransfert` vérifié en simulant un échec réseau : la modale reste ouverte, le bouton redevient cliquable, un message d'erreur s'affiche (contre un silence total et un blocage permanent auparavant).
- **Effet de bord découvert et corrigé en cours de route (sans lien avec le bug)** : la fragilité de `tests/dashboard.global-search.test.js` (ajouté en session précédente) face à un checkout en fin de ligne CRLF (`core.autocrlf=true` sous Windows) — un `git stash`/`pop` en cours de diagnostic a suffi à faire échouer le test, ses marqueurs multi-lignes étant des littéraux `\n` stricts. Corrigé en normalisant la source lue (`\r\n`→`\n`) avant extraction, même principe que le `\r?\n` déjà utilisé dans `security.gated-actions.test.js` pour la même raison. Vérifié en forçant artificiellement `dashboard.html` en CRLF puis en confirmant que le test passe toujours.
- `npm run verify` : 253/253 (aucun nouveau test — correctif de comportement pur, pas de nouvelle logique métier testable côté serveur ; le fichier de test existant est simplement rendu robuste au CRLF). `npm run sync:staging` relancé après modification de `dashboard.html`.

## Calcul et affichage du besoin annuel EPI (9 septembre 2026)

- **Première tranche du module « Besoin EPI »** cadré la veille (`CADRAGE_MODULE_BESOIN_EPI.md`) : produire en une minute le besoin de l'année N+1 par article et par taille, ventilé Réunion/Mayotte, avec la liste des données manquantes qui le rendraient faux. **Portée volontairement réduite par rapport au cadrage complet** : uniquement le calcul et l'affichage (écran « 📋 Besoin annuel », nouveau sous-onglet EPI placé après « 🛒 Suggestion de commande »), **aucune écriture SharePoint, aucune nouvelle liste, aucune nouvelle action Worker**. Le cadrage prévoyait une liste `Effectif_Previsionnel_EPI` persistée et une action `valider_besoin_consultation_epi` figeant le besoin (cadrage §7 session 2) — reporté : dans cette tranche, l'effectif prévisionnel (entrées/sorties par affectation × territoire) et la marge de sécurité sont des **paramètres éphémères**, saisis en mémoire et recalculés à la volée, jamais persistés. Les listes `Consultations_EPI`/`Fournisseurs_EPI`/`Lignes_Offres_EPI`/l'arbitrage (cadrage §4-§6) restent entièrement hors périmètre.
- **Règle structurante respectée : pas de 3e copie du mapping taille → article.** Il en existe déjà deux, volontairement (`TAILLE_FIELD_PAR_TYPE`/`trouverArticleCatalogue` dans `worker.js`, `epiCalculerBesoinStock`/`epiTrouverArticleCatalogue` dans `dashboard.html`, voir `CADRAGE_MODULE_BESOIN_EPI.md` §1.2) — le nouveau calcul (`epiCalculerBesoinAnnuel`) **réutilise `epiCalculerBesoinStock` telle quelle** pour la résolution catalogue par employé, et n'ajoute qu'une couche au-dessus : annualisation (renouvellement), ventilation territoriale, effectif prévisionnel, marge. Un nouveau test (`tests/dashboard.epi-besoin-annuel.test.js`) compare les deux tables de mapping (worker.js vs dashboard.html) après normalisation de la convention de nommage (`Taille_Pantalon` vs `taillePantalon`) et échoue si elles divergent — seul garde-fou réaliste contre une dérive silencieuse entre les deux runtimes (zéro build, donc pas de partage de code possible).
- **`calculerManquantsEPI` délibérément NON réutilisée/modifiée** malgré une première formulation de la demande qui le suggérait : cette fonction alimente déjà "Suggestion de commande" sur un périmètre différent et incompatible (uniquement les employés sans fiche cette année, sans annualisation) — la modifier aurait changé le comportement d'un écran existant déjà en production. Une fonction dédiée (`epiCalculerBesoinAnnuel`) a été écrite à la place, documentée en commentaire pour expliquer pourquoi les deux coexistent plutôt que d'être fusionnées.
- **Formule (ordre imposé, voir cadrage §2)** : dotation de base (`Grille_Dotation_EPI.Quantite`) → renouvellement (`CEIL(base × 12 / Renouvellement_Mois)`, défaut 12) → effectif prévisionnel (delta entrées−sorties **plafonné à 0** — un solde net de départs ne retire jamais de besoin déjà couvert par l'effectif réel) → marge de sécurité → **arrondi à l'unité supérieure au niveau de la ligne, par territoire séparément** (`CEIL(Total_Reunion×(1+marge))` et `CEIL(Total_Mayotte×(1+marge))` indépendamment, le total étant la somme des deux quantités déjà arrondies — jamais un arrondi global). Taille des entrées prévisionnelles (sans identité donc sans taille connue) : la taille **la plus fréquente** observée parmi les employés réels actuels du même profil/territoire ; si aucune référence réelle n'existe, anomalie "taille à préciser" plutôt qu'une taille devinée.
- **6 catégories d'anomalies, toutes affichées même vides** (bloc d'audit en tête d'écran, avec un compteur explicite "besoin calculé sur X employés éligibles sur Y — Z exclus") : taille manquante, article introuvable pour cette taille, type d'article absent du catalogue (anomalie de configuration, vérifiée une fois, pas par employé — zéro besoin calculé pour ce type tant que non corrigé), affectation EPI non reconnue, site non exploitable, et taille à préciser (entrées prévisionnelles sans référence de taille disponible). Une taille/article manquant exclut seulement la ligne concernée pour cet employé ; une affectation ou un site non exploitable exclut l'employé entièrement du calcul (compté "exclu"), les deux étant distingués dans le compteur d'effectif.
- **`Renouvellement_Mois` : prise en charge défensive de bout en bout.** Vérifié en direct sur la production (`?debug_columns=Grille_Dotation_EPI`, 9 sept. 2026) : colonne **absente**, William ne l'a pas encore créée. Le calcul retombe alors sur 12 partout, sans régression. Deux points étendus (pas de nouvelle action, l'action existante `maj_grille_dotation_epi` a été généralisée) : (1) l'endpoint `?grille_dotation_epi=1` projette désormais aussi `renouvellement_mois` (`null` si la colonne n'existe pas, jamais une erreur — `$expand=fields` sans `$select` ne dépend pas de l'existence d'une colonne) ; (2) `maj_grille_dotation_epi` accepte un champ optionnel `renouvellement_mois`, envoyé au serveur uniquement s'il est fourni et strictement positif, jamais écrasé avec une valeur vide. Écran d'édition de la grille de dotation (`renderEPIGrille`) complété d'un lien "🔁 X mois" sous chaque quantité, éditable via `prompt()` (même pattern que les autres réglages ponctuels du dashboard) — tant que la colonne n'existe pas côté SharePoint, la tentative d'édition échoue explicitement (message dédié), sans jamais rester silencieusement ouverte.
- **Aucune nouvelle capacité `ROLE_CAPS`/droit dashboard** : le nouvel écran ne porte aucune donnée tarifaire (contrairement aux volets "Fournisseurs & Offres"/"Arbitrage" du cadrage complet, qui introduiront `peutGererBesoinEPI`/`peutVoirBesoinEPI` en temps voulu) — réutilise `peutVoirEPI`/`peutGererEPI` déjà en place (Admin/Logistique/Logistique_Mayotte en édition, + Encadrement en lecture seule), cohérent avec le raisonnement du cadrage (§8.4) selon lequel un besoin en quantités seules reste une information utile à l'Encadrement.
- **Export** : `exporterBesoinAnnuelEPI()` réutilise `exporterExcel`/`ligneFiltresActifs`/`nomFichierExport` tels quels (aucun nouveau moteur d'export), 4 onglets comme demandé (Paramètres et hypothèses, Besoin par article, Détail par employé, Anomalies) — les paramètres/hypothèses de calcul (année, marge, effectif prévisionnel, effectif compté/exclu) sont toujours inclus, pour qu'une consultation ne soit jamais présentée sans les hypothèses qui l'ont produite.
