...
// Détecté par le CHEMIN de la page (ex. /immo-tracker/staging/index.html), jamais par un
// paramètre d'URL — un chemin persiste au rechargement/partage/marque-page, contrairement à un
// "?recette=1" qui se perd dès qu'on navigue sans le reporter. Ce chemin n'est jamais lié depuis
// la vraie app (aucune navigation interne n'y mène) : un utilisateur terrain normal ne peut donc
// jamais y atterrir par accident. Voir PROCEDURE_RECETTE.md pour le déploiement de /staging/.
const IS_RECETTE = typeof location !== 'undefined' && location.pathname.indexOf('/staging/') !== -1;

const CONFIG = {
  proxy:   IS_RECETTE ? 'https://immo-proxy-staging.ral-85d.workers.dev/' : 'https://immo-proxy.ral-85d.workers.dev/',
  // Admins / logistique
...
if (IS_RECETTE) {
  document.body.classList.add('recette-active');
  const rb = document.getElementById('recette-banner');
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length) {
        arr = data.map(e => ({ Code: e.code, Nom: e.nom, Poste: e.poste || '', Droits: e.role || '', Site: e.site || '', Actif: e.actif !== false }));
      }
    ...
    S.siteByCode = {};
    S.actifByCode = {};
    arr.forEach(e => { S.droitsByCode[e.Code] = e.Droits || ''; S.siteByCode[e.Code] = e.Site || ''; S.actifByCode[e.Code] = e.Actif !== false; });
    // Pour les sélecteurs de réception : exclure les employés inactifs (ne peuvent pas recevoir de matériel)
    const employesActifs = arr.filter(e => e.Actif !== false);
    ...
    selIds.forEach(selId => {
      ...
      sel.innerHTML = def;
      const coniFirst = employesActifs.filter(e => e.Code === 'CONI');
      const autres    = employesActifs.filter(e => e.Code !== 'CONI' && (!S.employe || e.Code !== S.employe.code));
      if (coniFirst.length) {
...
function activerEmploye(code, nom) {
  ...
  chargerEmployes().then(() => {
    // Bloquer l'accès si l'employé est inactif
    if (S.actifByCode && S.actifByCode[employe.code] === false) {
      localStorage.removeItem('employe');
      S.employe = null;
      const badge = document.getElementById('user-info');
