...
- Module Brasseurs d'air (négoce + pose) — mouvements, transferts inter-dépôts, commandes fournisseur avec réception et gestion des écarts, stock calculé par dépôt × référence × propriétaire (août 2026, code livré et testé — cadrage dans `Fichiers divers/Brasseurs d'air/CADRAGE_MODULE_BRASSEURS.md`, détail dans `02_MODELE_DONNEES.md`/`03_REGLES_METIER_ET_ROLES.md`/`04_HISTORIQUE_DECISIONS.md`. **Testé et validé uniquement en recette à ce stade** — synchronisé dans `staging/`, pas encore poussé/déployé en production, en attente de validation par William)
