...
74 actions POST (`?action=<nom>`), dispatchées après `const body = await request.json()`. Trois niveaux, vérifiés par grep sur chaque bloc `if (action === '...')`.

### `requireAdmin` (25 actions — admin strict)

`reset_password`, `maj_droits`, `maj_site_employe`, `ajouter_immo`, `importer_affectation_immo`, `ajouter_employe`, `bulk_patch_immos`, `dedupe_employes`, `maj_actif`, `seed_immo_sites`, `seed_roles_sites`, `bulk_import_lignes_inventaire`, `bulk_import_tailles_epi`, `bulk_import_catalogue_epi`, `bulk_import_grille_dotation_epi`, `bulk_import_catalogue_outillage`, `bulk_maj_duree_outillage`, `bulk_import_grille_outillage`, `bulk_import_lignes_outillage`, `ajouter_materiel_it`, `bulk_import_materiel_it`, `bulk_import_mouvements_materiel_it`, `ajouter_ligne_telephonique`, `bulk_import_lignes_telephoniques`, `bulk_import_mouvements_lignes_telephoniques`

*(Ces 25 sont des actions POST. `export_liste` est aussi protégé `requireAdmin` mais c'est un endpoint GET distinct — voir le tableau des endpoints GET plus bas dans cette section.)*

### `requireGarant` (Admin + Logistique + Logistique_Mayotte — 33 actions)

`upload_fds`, `maj_site_immo`, `marquer_statut_immo`, `maj_panne`, `enregistrer_reparation`, `creer_campagne_inventaire`, `cloturer_campagne_inventaire`, `maj_taille_employe`, `maj_grille_dotation_epi`, `supprimer_grille_dotation_epi`, `ajouter_article_epi`, `bulk_maj_stock_epi`, `reception_commande_epi`, `generer_dotation_epi`, `generer_remise_ponctuelle_epi`, `emarger_dotation_epi`, `upload_fiche_epi`, `annuler_dotation_epi`, `maj_service_outillage`, `maj_grille_outillage`, `supprimer_grille_outillage`, `reception_commande_outillage`, `editer_stock_outillage`, `distribuer_outillage`, `upload_fiche_outillage`, `annuler_ligne_outillage`, `maj_materiel_it`, `affecter_materiel_it`, `maj_ligne_telephonique`, `affecter_ligne_telephonique`, `annuler_transfert`, `maj_transfert`, `creer_mouvement_direct`

### Publiques — partagées avec la PWA terrain (badge sans mot de passe — 15 actions)

`verify_password`, `set_password` *(actions d'entrée qui produisent le jeton — ne peuvent pas déjà en exiger un)*, `upload_photo`, `delete_photo`, `reserver`, `statut_resa`, `modifier_resa`, `transfert`, `declarer_panne`, `resoudre_panne`, `signaler_absence`, `ajouter_ligne_inventaire`, `supprimer_ligne_inventaire`, `declarer_vol`, `valider`

⚠️ **`maj_duree_amort` — action non protégée, orpheline** : ce bloc existe dans `worker.js` et écrit encore la colonne `Duree_Amortissement`, mais **aucun appel n'existe plus dans `dashboard.html`** (grep : 0 résultat) et `02_MODELE_DONNEES.md` documente cette colonne comme inerte depuis le passage à la durée 100% automatique. C'est donc un endpoint d'écriture accessible sans authentification à quiconque connaît l'URL du Worker, mais inatteignable depuis l'interface. Voir § 7.
...
4. **Action Worker `maj_duree_amort` non protégée et orpheline** : aucun appel côté `dashboard.html`, mais toujours présente et exécutable dans `worker.js` sans authentification, alors que `02_MODELE_DONNEES.md`/`04_HISTORIQUE_DECISIONS.md` décrivent la durée d'amortissement comme 100% automatique depuis juillet 2026 et la fonctionnalité de saisie manuelle comme retirée. Risque mineur (colonne inerte pour les calculs actuels) mais reste un endpoint d'écriture public non documenté comme tel — à clore explicitement (suppression du bloc) dans une session dédiée, non traité ici pour respecter la consigne « ne modifie aucun comportement métier existant ».
